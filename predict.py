	...
	for idx in range(num_anchors_for_bbox.shape[0]):
		if num_anchors_for_bbox[idx] == 0:
			...
			y_rpn_overlap[
				best_anchor_for_bbox[idx, 0], best_anchor_for_bbox[idx, 1], best_anchor_for_bbox[idx, 2] + n_anchratios *
				best_anchor_for_bbox[idx, 3]] = 1
			start = 4 * (best_anchor_for_bbox[idx, 2] +
...
config_output_filename = os.path.join(
	'Trained Model/model_vgg_config.pickle')

with open('Trained Model/model_vgg_config.pickle', 'rb') as f_in:
	C = pickle.load(f_in)
	...
	if count == 100:
		...
		for key in bboxes:
			...
			for jk in range(new_boxes.shape[0]):
				...
				cv2.rectangle(frame, (real_x1, real_y1), (real_x2, real_y2), (int(
					class_to_color[key][0]), int(class_to_color[key][1]), int(class_to_color[key][2])), 4)
				...
				cv2.rectangle(frame, (textOrg[0] - 5, textOrg[1]+baseLine - 5),
							(textOrg[0]+retval[0] + 5, textOrg[1]-retval[1] - 5), (0, 0, 0), 1)
				cv2.rectangle(frame, (textOrg[0] - 5, textOrg[1]+baseLine - 5),
							(textOrg[0]+retval[0] + 5, textOrg[1]-retval[1] - 5), (255, 255, 255), -1)
				cv2.putText(frame, textLabel, textOrg,
							cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 0), 1)

		print('Elapsed time = {}'.format(time.time() - st))
		print(all_dets)
		cv2.imshow('frame', frame)
		cv2.imwrite("predict_output_1.jpg", frame)
		break
	else:
